    ...
    def load_transform_file(self, file_path):
        ...
        with self.driver.session() as session:
            for _, row in trips.iterrows():
                ...
                query = """
                MERGE (pickup:Location {name: toInteger($pickup_id)})
                MERGE (dropoff:Location {name: toInteger($dropoff_id)})
                CREATE (pickup)-[trip:TRIP]->(dropoff)
                SET trip.distance = toFloat($distance),
                    trip.fare = toFloat($fare),
                    trip.pickup_dt = datetime($pickup_dt),
                    trip.dropoff_dt = datetime($dropoff_dt);
                """
                session.run(query, {"pickup_id": pickup_id, "dropoff_id": dropoff_id, "distance": distance, "fare": fare, "pickup_dt": pickup_dt, "dropoff_dt": dropoff_dt})
